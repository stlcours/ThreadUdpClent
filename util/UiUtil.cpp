...
// 按内容显示标题
void UiUtil::setTabViewColumnSpace(QTableView *view)
{
    //根据内容来确定列宽度
    ...
    {
        view->setColumnWidth(i, view->columnWidth(i) );
    }
    //    view->horizontalHeader()->setStretchLastSection(true); //5.6
...
// 查找 w 所在窗口的顶层窗口
QWidget *UiUtil::findParentWindow(QWidget *w) {
    QWidget *p = w;
...
//依靠写文件与移除文件实现单例程序运行
bool UiUtil::mySingleApp()
{
    bool isInstance =true;
    QFile file;
    file.setFileName("configsoft.lck");
    if(file.exists()){
        isInstance=file.remove(); //true
        if(false == isInstance){
#if 0
            QMessageBox message(QMessageBox::NoIcon, tr("exit"),
                                tr("A copy of the software is running,"
                                   "the software will be exit "), QMessageBox::Yes );
            message.exec();
#endif
            isInstance =false;
        }else{
...
void UiUtil::QRegExp_ipv4(QLineEdit *lineEdit)
{
    lineEdit->setValidator(new QIntValidator(1,32767));
    QRegExp rx2("\\b(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\b");
    //    lineEdit->setValidator(new QRegExpValidator(rx2, this));
    lineEdit->setValidator(new QRegExpValidator(rx2));
}
void UiUtil::QRegExp_Int_Rang(QLineEdit *lineEdit,int low,int high)
{
    lineEdit->setValidator(new QIntValidator(low,high));
}
